  ...
  <Row className="gutter-row">
		...
		<Col md={22} className="gutter-col">
			<Card title="componentWillReceiveProps">
				<p>在组件接收到一个新的prop（更新后）时被调用。这个方法在初始化render时不会被调用</p>
			</Card>
		</Col>
		<Col md={22} className="gutter-col">
			<Card title="shouldComponentUpdate">
				<p>返回一个布尔值。在组件接受到新的props或者state时被调用。返回true就是允许更新，返回false就是不允许更新，把决定权交给开发者</p>
			</Card>
		</Col>
		<Col md={22} className="gutter-col">
			<Card title="componentWillUpdate">
				<p>在组件接收到props或者state但还有render时被调用。在初始化时不会被调用</p>
			</Card>
		</Col>
		<Col md={22} className="gutter-col">
